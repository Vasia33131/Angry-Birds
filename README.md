Angry Birds на Unity — заряжай рогатку, круши зелёных свиней и разрушай постройки!
Главное меню | Игровой процесс | Разрушаемые конструкции и боссы

<img width="1267" height="711" alt="Снимок экрана 2026-05-14 200822" src="https://github.com/user-attachments/assets/ee6404f1-8a0f-4c0f-8a3a-f4f1c106d9b7" />
<img width="585" height="329" alt="Снимок экрана 2026-05-14 200805" src="https://github.com/user-attachments/assets/c272dd96-ab52-48cc-816b-29ca97abdfa0" />




https://github.com/user-attachments/assets/f9731b3e-49f2-410a-83fb-e0e79cced9f8



О проекте
Жанр: Arcade / Physics Puzzle
Движок: Unity
Язык: C#
Платформа: WebGL / PC / Mobile

Управление
Зажать ЛКМ / тач — натянуть рогатку (тяни назад)

Отпустить — выстрел птицей

Мышь / палец — прицеливание (траектория полёта отображается пунктиром)


Физика разрушений (Unity Physics 2D)

Звёзды за уровень (1–3 звезды в зависимости от очков)

Постройки из дерева, камня, стекла, льда

Автор
Vasia33131
